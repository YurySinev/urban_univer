...
#
#
# К сожалению, у вас не так много времени, чтобы подбирать пароль вручную, шипы сверху уже
# движутся на вас (обожаю клише), тем более числа в первой вставке будут попадаться случайно.
#
# Составьте алгоритм, используя циклы, чтобы вне зависимости от введённого числа n (от 3 до 20)
...
def calc_password(num):
    ...
    for i in range(1, half_num + 1):
        for j in range(1, num + 1):
            if j <= i:  # второе число д.быть больше первого
                continue
...
print(calc_password(first_num))  # передаем его в функцию для вычисления пароля и выводим результат
