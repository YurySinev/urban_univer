...
# матрицу(вложенный список) размерами n строк и m столбцов, заполненную значениями value
# и возвращать эту матрицу в качестве результата работы.
#
# Пункты задачи:
...
def get_matrix(n: int, m: int, value: int) -> list:
    list_ = [[value for _ in range(m)] for _ in range(n)]
    return list_
