...
# Важное примечание! Я назвал поначалу этот файл os.py и не мог понять, почему у меня не работают
# подсказки в PyCharm, и почему не работает команда os.startfile(path). Сначала я решил, что это,
# может, особенности МакОС.
# И сейчас до меня дошло, что назвав свой файл именем, совпадающим с системным модулем os, я его
# ПЕРЕОПРЕДЕЛИЛ! И другие мои файлы в этой же папке, видя строку import os, импортировали вот этот
# файл-пустышку, а не настоящий модуль os. Отсюда и все проблемы. Пришлось переименовывать и исправлять
# неверные ссылки.
