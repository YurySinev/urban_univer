...
#
# Цель: применить на практике начальные знания о пространстве имён и
# оператор global. Закрепить навыки из предыдущих модулей.
#
...
def count_calls() -> None:  # подсчитывает вызовы остальных функций
    global calls
    calls += 1


def string_info(line: str) -> tuple:  # принимает аргумент - строку
    count_calls()
...
print(is_contains('Urban', ['ban', 'BaNaN', 'urBAN']))  # Urban ~ urBAN
print(is_contains('cycle', ['recycling', 'cyclic']))  # No matches
print(string_info('Играй, гармонь!'))
print(is_contains('Меркурий',
                  ['ЗЕМЛЯ', "МАРС", "ВЕНЕРА", "ЮПИТЕР", "САТУРН", "МЕРКУРИЙ", "ПЛУТОН", "ЛУНА", "ЦЕРЕРА", "ЕВРОПА",
                   "КЛИО"]))
print(calls)
